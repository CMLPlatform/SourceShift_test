...
#def shift_source():
#    print('sourceshift')
#    dict_cntr_short_long = ut.get_dict_cntr_short_long()
#
##    x_prod_cntr_min = 0.5
#
#    dict_df_imp_pME = {}
...
#    dict_imp_pME = ut.get_dict_imp(dict_df_imp_pME)
#
...
def calc_new_fp():
    ...
    for imp_cat_id, imp_cat in enumerate(source_shift.dict_y_new):
        dict_df_tY_import_new[imp_cat] = {}
        for prod in source_shift.dict_y_new[imp_cat]:
            for cntr in source_shift.dict_y_new[imp_cat][prod]:
                dict_df_tY_import_new[imp_cat][(cntr, prod)] = (
                        source_shift.dict_y_new[imp_cat][prod][cntr])
    ...
    for imp_cat_id, imp_cat_sel in enumerate(dict_df_tY_import_new):
        ...
        dict_df_imp_new = ut.get_dict_df_imp(dict_cf, dict_eb, df_tY_eu28_import)
        dict_imp_new = ut.get_dict_imp(dict_df_imp_new)
...
def calc_improv():
    ...
    dict_imp_cat_unit = ut.get_dict_imp_cat_unit()
    list_prod_order_cons = ut.get_list_prod_order_cons()
    plt.close('all')
    fig = plt.figure(figsize=ut.cm2inch((16, 1+13*0.4)))
    dict_imp_prod_sort_full = ut.get_dict_imp_prod_sort(dict_df_imp, cfg.
                                                         imp_cum_lim_full)
...
def calc_improv_agg():
    ...
    dict_imp_prod_sort_full = ut.get_dict_imp_prod_sort(dict_df_imp,
...
dict_imp_cat_magnitude = ut.get_dict_imp_cat_magnitude()
dict_imp_cat_fp = ut.get_dict_imp_cat_fp()
dict_prod_long_short = ut.get_dict_prod_long_short()

dict_eb = ut.get_dict_eb()

dict_cf = ut.get_dict_cf(dict_eb)
list_reg_fd = ut.get_list_reg_fd('EU28')
df_tY_eu28 = dict_eb['tY'][list_reg_fd].copy()

for cntr in list_reg_fd:
    df_tY_eu28.loc[cntr, cntr] = 0
df_tY_eu28_fdsum = df_tY_eu28.sum(axis=1)
dict_df_imp = ut.get_dict_df_imp(dict_cf, dict_eb, df_tY_eu28_fdsum)
dict_imp = ut.get_dict_imp(dict_df_imp)

ut.make_result_dir()
#dict_priority = ut.calc_priority(dict_df_imp, cfg.imp_cum_lim_priority)
#ut.plot_priority(dict_priority)
#ut.log_priority(dict_priority)
#set_priority()
#dict_y_new = shift_source()

priority = ut.Priority()
priority.calc(dict_df_imp, cfg.imp_cum_lim_priority)
priority.plot()
priority.log()

source_shift = ut.SourceShift()
source_shift.calc(dict_cf, dict_eb, dict_imp, df_tY_eu28)
source_shift.plot(dict_cf, dict_eb, dict_imp, df_tY_eu28)
source_shift.log()
